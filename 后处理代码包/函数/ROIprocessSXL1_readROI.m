function ROIprocessSXL1_readROI(nROI,ROIpath,w_offset,x_lw,Savepath,ROI_total)
load(fullfile(ROIpath, 'ROI_total'));
load(fullfile(Savepath,['V_as.mat']));
load(fullfile(Savepath,['V_exp.mat']));
N_echo=1;%%暂取echo=1.
x_as=[min(w_offset):x_lw:max(w_offset)];%插值
[Nx,Ny,Nas]= size(V_as);
Nw = length(w_offset);
...
for k=1:nROI
    ...
    if min(w_offset) < 0
         MTRasymM0_as(:,k)= mSig(1:(Nas-1)/2,k)- mSig(Nas:-1:((Nas+3)/2),k);
